 ...
 * INPUTS: digitNum (Integer), displayNum (Integer)
 * RETURN: none
 * PURPOSE: Takes the target digit (Integer) and a number (Integer) to print to that digit
 */
 ...
 * INPUTS: digitNum (Integer), number[] (Integer Array)
 * RETURN: none
 * PURPOSE: Takes the target digit (Integer) and a number (Integer Array)
 * 			and displayes the desired number on the desired digit. The digit
 * 			consists of an array of booleans that tells respective segments
 * 			to switch on and off. 
 */
void displaySegment(uint8_t digitNum, uint8_t number[])
{
	/*
	 * This digit array is a temporary storage for the desired digit
	 * eg digit1 etc. This holds the start pixels for the segments. 
	 */
	int digit[7];

	/*
	 * This switch case fills the temporary array with the start values
	 * from the SEGMENT MAPPINGS. 
	 */
	switch (digitNum)
	{
		case 1: 
	...
	/*
	 * This section fills a buffer (Global Variable) array
	 * with boolean values telling segments to turn on or off. 
	 */
	for(int i = 0; i < 7; i++)
	{
		if(number[i] == 1)
 ...
 * INPUTS: numDots (Integer)
 * RETURN: none
 * PURPOSE: Allows a single decimal dot or a colon to be shown
 */
void displayDots(uint8_t numDots)
{
	if(numDots == 1)
	{
		ledBuffer[0] = 0;
		ledBuffer[stripLength - 1] = 1;
	}
	else
	{
		ledBuffer[0] = 1;
 ...
 * INPUTS: none
 * RETURN: none
 * PURPOSE: Takes the global array "ledBuffer" and outputs it
 * 			to the LED strip all at once. It sends the set colour
 * 			to all leds that are marked to turn on. 
 */
void displayBuffer(uint8_t numberColour[], uint8_t dotColour[])
{
	for(int i = 0; i < stripLength; i++)
	{
		if(ledBuffer[i] == 1)
		{
			if(i == 0 || i == (stripLength - 1))
			{
				cli();
				sendPixel(dotColour[0],dotColour[1],dotColour[2]);
				sei();
			}
			else
			{
				cli();
				sendPixel(numberColour[0],numberColour[1],numberColour[2]);
				sei();
			}
		}
		else
...
/*
 * FOLLOWING CODE IS MODIFIED FROM INTERNET SOURCE
 */
...
#define PIXELS stripLength  // Number of pixels in the string
